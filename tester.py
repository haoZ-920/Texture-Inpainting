...
def WGAN_tester(opt):
    ...
    def load_trained_model():
        model_name = 'models/deepfillv2_WGAN_G_epoch100_batchsize6.pth' # epoch40_batchsize6
        pretrained_dict = torch.load(model_name)
        generator.load_state_dict(pretrained_dict)
        pass
    ...
    rend = Render(img_size=[opt.imgsize,opt.imgsize], batch_size=opt.batch_size, device=device)

    # ----------------------------------------
    #            Testing
    # ----------------------------------------
    # Testing loop
    for batch_idx, (img, mask, verts) in enumerate(dataloader):
        img = img.cuda()
        mask = mask.cuda()
        verts = verts.cuda()
        ...
        second_out_wholeimg =  second_out #img * (1 - mask) + second_out * mask      # in range [0, 1]

        rend.load_texture(second_out_wholeimg)
        renderd = rend.get_renderd_tensor(verts)
