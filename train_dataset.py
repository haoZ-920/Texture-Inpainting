...
class InpaintDataset(Dataset):
    def __init__(self, opt):
        ...
        unwrap_img_dir = opt.baseroot + "/gt_img_src" # unwrap
        gt_img__dir = opt.baseroot + "/gt_img"
        mask_img_dir = opt.baseroot + "/mask"
        pkldata_dir = opt.baseroot + "/train_param"

        self.imglist = sorted(utils.get_files(unwrap_img_dir)) # opt.baseroot
        self.gtimglist = sorted(utils.get_files(gt_img__dir))
        self.maskimglist = sorted(utils.get_files(mask_img_dir))

        self.pkldatalist = sorted(utils.get_files(pkldata_dir)) #"./test_param"
    ...
    def __getitem__(self, index):
        ...
        gtimg = cv2.imread(self.gtimglist[index])
        gtimg = cv2.resize( gtimg, shape , interpolation = cv2.INTER_AREA)
        gtimg = cv2.cvtColor(gtimg, cv2.COLOR_BGR2RGB)

        maskimg = cv2.imread(self.maskimglist[index])[:, :, 0]
        maskimg = cv2.resize(maskimg, shape, interpolation=cv2.INTER_AREA)
        ...
        gtimg = torch.from_numpy(gtimg.astype(np.float32) / 255.0).permute(2, 0, 1).contiguous()
        maskimg = torch.from_numpy(maskimg.astype(np.float32) / 255.0).unsqueeze(0).contiguous()
#         mask = torch.from_numpy(mask.astype(np.float32)).contiguous()
#         mask = self.random_mask()[0]

        with open(self.pkldatalist[index], 'rb') as file:
            pkldata = pkl.load(file )
        verts = torch.from_numpy(pkldata['vertices']).contiguous()


        return img, gtimg, maskimg, verts #, height, width
